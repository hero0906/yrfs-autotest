...
def yrfs_update(config="reboot"):
    ...
    if version_res == "6.6":
        ...
        stop = ["systemctl daemon-reload","umount -l /mnt/yrfs","/etc/init.d/yrfs-client stop","systemctl stop yrfs-mgr","systemctl stop yrfs-oss","systemctl stop yrfs-agent"]
        client_ver = ['grep "yrfs client version" /var/log/messages|tail -n 1']
    ...
    if version_stat == 0: 
        ...
        reboot_web = ["systemctl restart yrcloudfile-dashboard","service inert-krypton-host restart",\
                "systemctl restart inert-iron-scheduler","systemctl restart inert-iron-scheduler-beat","systemctl
                restart inert-krypton-api"]

        reboot_pcs = "pcs resource restart krypton-api-clone&&pcs resource restart krypton-cluster&&su -s /bin/sh -c \"krypton-dbsync\" krypton "
    ...
    centos_rpm = ["wget -c -r -nd -np -k -L -A rpm http://192.168.0.22:17282/v66x-daily/daily-build/rpms/ -P "\
            "/autotest_rpm/%s > /dev/null 2>&1" % time_dir,"rpm -Uvh /autotest_rpm/%s/yrfs-*.rpm" % time_dir,"systemctl daemon-reload"] 

    ubuntu_rpm = ["wget -c -r -nd -np -k -L -A yrfs*deb http://192.168.0.22:17282/v66x-daily/daily-build/clients/ubuntu2004/ -P "\
            "/autotest_rpm/%s > /dev/null 2>&1" % time_dir, "dpkg -i /autotest_rpm/%s/yrfs-*.deb" % time_dir,"systemctl daemon-reload"]

    client_update_cmd = centos_rpm + kill_mnt + ["/etc/init.d/yrfs-client stop","/etc/init.d/yrfs-client start"]
    ...
    if config == "reboot": 
        ...
        return(reboot_server, reboot_client, reboot_pcs)

    else:
        print(ctime() + '\033|choice command: update yrfs service.\n\033[0m')
        return(server_update_cmd, client_update_cmd, reboot_pcs)
